...
export default function OrdersPage() {
  const [activeTab, setActiveTab] = useState<'user' | 'reseller'>('user')
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab Content */}
      {activeTab === 'user' ? <UserOrdersTab /> : <ResellerOrdersTab />}
    </div>
  )
}
...
function ResellerOrdersTab() {
  const supabase = createBrowserClient()
  const [orders, setOrders] = useState<ResellerOrder[]>([])
  const [resellers, setResellers] = useState<Reseller[]>([])
  const [orderItems, setOrderItems] = useState<Record<string, ResellerOrderItem[]>>({})
  const [loading, setLoading] = useState(true)
  const [searchQuery, setSearchQuery] = useState('')
  const [statusFilter, setStatusFilter] = useState<string>('all')
  ...
  useEffect(() => {
    fetchResellerOrders()
    const interval = setInterval(() => fetchResellerOrders(), 30_000)
    const channel = supabase
      .channel('reseller-orders-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'reseller_orders' }, () => fetchResellerOrders())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'reseller_order_items' }, () => fetchResellerOrders())
      .subscribe()
    return () => {
      clearInterval(interval)
      supabase.removeChannel(channel)
  ...
  useEffect(() => { setCurrentPage(1) }, [searchQuery, statusFilter, resellerFilter, dateFilterType, dateValue, monthValue, yearValue])
  useEffect(() => {
    const timer = setInterval(() => setNowTs(Date.now()), 60000)
    return () => clearInterval(timer)
  }, [])
  ...
  const fetchResellerOrders = async () => {
    try {
      const resp = await fetch('/api/reseller-orders', { cache: 'no-store' })
      const payload = await resp.json()
      if (!resp.ok) throw new Error(payload?.error || 'Failed to fetch reseller orders')
      setOrders(payload?.orders || [])
      setResellers(payload?.resellers || [])
      setOrderItems(payload?.orderItems || {})
    } catch (error) {
  ...
  const getEffectiveStatus = (order: ResellerOrder) => {
    if (order.status !== 'pending') return order.status
    const createdAtMs = new Date(order.created_at).getTime()
    const expiresAtMs = createdAtMs + 15 * 60 * 1000
    return nowTs > expiresAtMs ? 'expired' : 'pending'
  ...
  const getResellerName = (resellerId: string) => {
    const r = resellers.find(rs => rs.id === resellerId)
    return r?.nama_toko || 'Unknown'
  }

  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      const searchLower = searchQuery.toLowerCase()
      const resellerName = getResellerName(order.reseller_id).toLowerCase()
      const matchesSearch =
        String(order.order_id || '').toLowerCase().includes(searchLower) ||
      ...
      const effectiveStatus = getEffectiveStatus(order)
      const matchesStatus = statusFilter === 'all' || effectiveStatus === statusFilter
      const matchesReseller = resellerFilter === 'all' || order.reseller_id === resellerFilter
      ...
      return matchesSearch && matchesStatus && matchesReseller && matchesDate
    })
  }, [orders, searchQuery, statusFilter, resellerFilter, dateFilterType, dateValue, monthValue, yearValue, nowTs, resellers])
  ...
  const allStats = {
    ...
    expired: orders.filter(o => getEffectiveStatus(o) === 'expired').length,
    revenue: orders.filter(o => getEffectiveStatus(o) === 'completed').reduce((sum, o) => sum + Number(o.total_amount || 0), 0),
    totalKomisi: orders.filter(o => getEffectiveStatus(o) === 'completed').reduce((sum, o) => sum + Number(o.komisi || 0), 0),
  }

  const isFiltered = searchQuery || statusFilter !== 'all' || resellerFilter !== 'all' || dateFilterType !== 'all'
  const stats = isFiltered ? {
    total: filteredOrders.length,
    pending: filteredOrders.filter(o => getEffectiveStatus(o) === 'pending').length,
    completed: filteredOrders.filter(o => getEffectiveStatus(o) === 'completed').length,
    expired: filteredOrders.filter(o => getEffectiveStatus(o) === 'expired').length,
    revenue: filteredOrders.filter(o => getEffectiveStatus(o) === 'completed').reduce((sum, o) => sum + Number(o.total_amount || 0), 0),
    totalKomisi: filteredOrders.filter(o => getEffectiveStatus(o) === 'completed').reduce((sum, o) => sum + Number(o.komisi || 0), 0),
  } : allStats
  ...
  if (loading) return <div className="text-center py-8">Loading reseller orders...</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
        <div className="bg-white rounded-lg shadow p-4 md:p-6 border-l-4 border-indigo-600 hover:shadow-lg transition">
          <p className="text-gray-600 text-xs md:text-sm font-medium">Total Orders</p>
          <p className="text-xl md:text-2xl font-bold text-gray-900 mt-1">{allStats.total}</p>
          {isFiltered && <p className="text-xs text-indigo-600 mt-1">Menampilkan: {stats.total}</p>}
        </div>
        <div className="bg-white rounded-lg shadow p-4 md:p-6 border-l-4 border-yellow-500 hover:shadow-lg transition">
          <p className="text-yellow-700 text-xs md:text-sm font-medium">Pending</p>
          <p className="text-xl md:text-2xl font-bold text-yellow-600 mt-1">{isFiltered ? stats.pending : allStats.pending}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 md:p-6 border-l-4 border-green-600 hover:shadow-lg transition">
          <p className="text-green-700 text-xs md:text-sm font-medium">Completed</p>
          <p className="text-xl md:text-2xl font-bold text-green-600 mt-1">{isFiltered ? stats.completed : allStats.completed}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 md:p-6 border-l-4 border-blue-600 hover:shadow-lg transition">
          <p className="text-blue-700 text-xs md:text-sm font-medium">Revenue</p>
          <p className="text-lg md:text-xl font-bold text-blue-600 mt-1">Rp {(isFiltered ? stats.revenue : allStats.revenue).toLocaleString('id-ID')}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 md:p-6 border-l-4 border-purple-600 hover:shadow-lg transition">
          <p className="text-purple-700 text-xs md:text-sm font-medium">Total Komisi</p>
          <p className="text-lg md:text-xl font-bold text-purple-600 mt-1">Rp {(isFiltered ? stats.totalKomisi : allStats.totalKomisi).toLocaleString('id-ID')}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 md:gap-4 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <div className="relative">
            <FiSearch className="absolute left-3 top-3 text-gray-400" />
            <input type="text" placeholder="Search order ID, customer, reseller..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
        </div>
        <select value={resellerFilter} onChange={(e) => setResellerFilter(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
          <option value="all">All Resellers</option>
          {resellers.map(r => <option key={r.id} value={r.id}>{r.nama_toko}</option>)}
        </select>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
          <option value="all">All Status</option>
          <option value="pending">Pending</option>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {filteredOrders.length === 0 ? (
          <div className="text-center py-12 text-gray-500">{orders.length === 0 ? 'No reseller orders yet' : 'No orders match your filters'}</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left px-4 py-3 w-12"></th>
                  <th className="text-left px-5 py-3 font-semibold text-gray-900">Order ID</th>
                  <th className="text-left px-5 py-3 font-semibold text-gray-900">Reseller</th>
                  <th className="text-left px-5 py-3 font-semibold text-gray-900">Customer</th>
                  <th className="text-right px-5 py-3 font-semibold text-gray-900">Total</th>
                  <th className="text-right px-5 py-3 font-semibold text-gray-900">Komisi</th>
                  <th className="text-center px-5 py-3 font-semibold text-gray-900">Items</th>
                  <th className="text-center px-5 py-3 font-semibold text-gray-900">Status</th>
                  <th className="text-left px-5 py-3 font-semibold text-gray-900">Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {paginatedOrders.map((order) => (
                  <Fragment key={order.order_id}>
                    <tr className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-5 py-3"><span className="font-mono text-sm font-medium text-gray-900">{order.order_id}</span></td>
                      <td className="px-5 py-3">
                        <span className="text-sm font-medium text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded">{getResellerName(order.reseller_id)}</span>
                      </td>
                      <td className="px-5 py-3 text-sm text-gray-600">
                        <div>{order.customer_name || '-'}</div>
                        {order.customer_email && <div className="text-xs text-gray-400">{order.customer_email}</div>}
                      </td>
                      <td className="px-5 py-3 text-right"><span className="font-semibold text-gray-900">Rp {Number(order.total_amount || 0).toLocaleString('id-ID')}</span></td>
                      <td className="px-5 py-3 text-right"><span className="font-medium text-purple-700">Rp {Number(order.komisi || 0).toLocaleString('id-ID')}</span></td>
                      <td className="px-5 py-3 text-center"><span className="inline-block bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-sm font-semibold">{getItemCount(order.id)}</span></td>
                      <td className="px-5 py-3 text-center"><span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(getEffectiveStatus(order))}`}>{getEffectiveStatus(order).charAt(0).toUpperCase() + getEffectiveStatus(order).slice(1)}</span></td>
                      <td className="px-5 py-3 text-sm text-gray-600">{new Date(order.created_at).toLocaleDateString('id-ID')} • {new Date(order.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false })}</td>
                    </tr>
                    {expandedOrderId === order.order_id && (
                      <tr className="bg-gray-50 border-t-2 border-gray-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="bg-white border border-gray-200 rounded-lg p-4">
                                ...
                                <div className="grid grid-cols-1 gap-2 text-sm">
                                  <div><span className="text-xs text-gray-500">Name:</span> <span className="text-gray-900">{order.customer_name || '-'}</span></div>
                                  <div><span className="text-xs text-gray-500">Email:</span> <span className="text-gray-900">{order.customer_email || '-'}</span></div>
                                  <div><span className="text-xs text-gray-500">Phone:</span> <span className="text-gray-900">{order.customer_phone || '-'}</span></div>
                                </div>
                              </div>
                              <div className="bg-white border border-gray-200 rounded-lg p-4">
                                <h5 className="text-sm font-semibold text-gray-900 mb-2">Order Summary</h5>
                                <div className="grid grid-cols-1 gap-2 text-sm">
                                  <div><span className="text-xs text-gray-500">Reseller:</span> <span className="text-gray-900 font-medium">{getResellerName(order.reseller_id)}</span></div>
                                  <div><span className="text-xs text-gray-500">Total Amount:</span> <span className="text-gray-900">Rp {Number(order.total_amount || 0).toLocaleString('id-ID')}</span></div>
                                  <div><span className="text-xs text-gray-500">Modal:</span> <span className="text-gray-900">Rp {Number(order.total_modal || 0).toLocaleString('id-ID')}</span></div>
                                  <div><span className="text-xs text-gray-500">Komisi Reseller:</span> <span className="text-purple-700 font-medium">Rp {Number(order.komisi || 0).toLocaleString('id-ID')}</span></div>
                                </div>
                              </div>
                            </div>
                              ...
                              {(orderItems[order.id] || []).length > 0 ? (
                                <div className="space-y-4">
                                  {(orderItems[order.id] || []).map((item, idx) => (
                                    <div key={idx} className={`pb-4 ${idx !== (orderItems[order.id] || []).length - 1 ? 'border-b border-gray-200' : ''}`}>
                                      <div className="flex justify-between items-start">
                                        <div>
                                          <p className="text-gray-900 font-semibold">{item.product_name}</p>
                                          {item.product_code && <p className="text-gray-600 text-xs">Code: {item.product_code}</p>}
                                        </div>
                                        <div className="text-right text-xs">
                                          <p className="text-gray-900 font-medium">Jual: Rp {Number(item.harga_jual || 0).toLocaleString('id-ID')}</p>
                                          <p className="text-gray-500">Modal: Rp {Number(item.harga_modal || 0).toLocaleString('id-ID')}</p>
                                        </div>
                                      </div>
                                      {item.item_data && (
                                        <div className="mt-2">
                                          <div className="flex items-center gap-2 mb-1">
                                            <p className="text-gray-500 text-xs font-medium">Item Data</p>
                                            <button onClick={() => handleCopy(item.item_data || '')} className={`text-[11px] font-medium px-2 py-0.5 rounded border ${copiedText === item.item_data ? 'bg-green-50 text-green-700 border-green-200' : 'bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100'}`}>{copiedText === item.item_data ? 'Copied' : 'Copy'}</button>
                                          </div>
                                          <code className="text-xs bg-gray-50 px-3 py-2 rounded block break-all text-gray-700 whitespace-pre-wrap border border-gray-200">{item.item_data}</code>
                                        </div>
                                      )}
                                    </div>
                                  ))}
                                ...
                                <p className="text-gray-500 text-sm">{getEffectiveStatus(order) === 'completed' ? 'Items delivered' : 'Waiting for payment'}</p>
                              )}
                            </div>
                          </div>
                        </td>
