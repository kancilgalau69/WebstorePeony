...
import { FiPlus, FiTrash2, FiCopy, FiKey, FiCheckCircle, FiClock, FiRefreshCw } from 'react-icons/fi'
...
type Stats = { total: number; unused: number; used: number }
type Toast = { type: 'success' | 'error'; message: string }

export default function RegistrationTokensPage() {
  const [tokens, setTokens] = useState<Token[]>([])
  const [stats, setStats] = useState<Stats>({ total: 0, unused: 0, used: 0 })
  ...
  const fetchTokens = useCallback(async () => {
    try {
      ...
      setStats(data.stats || { total: 0, unused: 0, used: 0 })
    } catch (err: any) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/registration-tokens', {
        ...
        body: JSON.stringify({ count: parseInt(count, 10) || 1, note }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Gagal generate token')
      showToast({ type: 'success', message: `${data.tokens?.length || 0} token berhasil dibuat` })
      setNote('')
      setCount('1')
      fetchTokens()
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-lg shadow p-4 border-t-4 border-green-500">
          <p className="text-xs text-gray-500 font-medium flex items-center gap-1"><FiClock /> Belum Dipakai</p>
        ...
        <div className="bg-white rounded-lg shadow p-4 border-t-4 border-gray-400">
          <p className="text-xs text-gray-500 font-medium flex items-center gap-1"><FiCheckCircle /> Sudah Dipakai</p>
                  ...
                  <tr key={t.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${
                        ...
                        {t.status === 'unused' ? 'Belum dipakai' : 'Sudah dipakai'}
                      </span>
                    </td>
