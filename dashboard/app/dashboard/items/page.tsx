...
import { FiPlus, FiTrash2, FiSearch, FiX, FiCopy, FiCheck, FiDownload, FiCheckSquare, FiSquare, FiChevronLeft, FiChevronRight, FiInfo, FiChevronDown } from 'react-icons/fi'
import type { Database } from '@/lib/database.types'

type Product = Database['public']['Tables']['products']['Row']
type ProductItem = Database['public']['Tables']['product_items']['Row']
type ProductItemInsert = Database['public']['Tables']['product_items']['Insert']

type StatusFilter = 'all' | 'available' | 'reserved' | 'sold'

type ProductItemOrderDetail = {
  id: string
  order_id: string
...
type ProductItemDetail = {
  item: ProductItem
  product: Pick<Product, 'id' | 'kode' | 'nama'> | null
  order: ProductItemOrderDetail | null
}
...
const formatItemDataForCopy = (itemData: string) => itemData.split('||').map(part => part.trim()).join('\n')
...
export default function ProductItemsPage() {
  ...
  const [selectedDetail, setSelectedDetail] = useState<ProductItemDetail | null>(null)

  useEffect(() => {
    fetchProducts()
    // Auto-refresh product list every 30 seconds as fallback
    // Skip refresh when a modal is open to prevent UI disruption
  ...
  }, [selectedProduct, showAddModal, showUploadModal])
  ...
  const fetchProducts = async () => {
    try {
      ...
      setProducts(data || [])
      // Only set selectedProduct on initial load (when no product is selected yet)
      setSelectedProduct(prev => {
        if (prev) return prev // Keep current selection during refresh
  ...
  const handleAddItems = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase
        .from('product_items')
        .insert(itemsToInsert satisfies ProductItemInsert[])
      ...
      alert('Failed to add items: ' + getErrorMessage(error))
    }
  }
  ...
  const handleCopyItemDataOnly = (itemData: string, id: string) => {
    navigator.clipboard.writeText(formatItemDataForCopy(itemData))
    setCopiedId(id)
    setTimeout(() => setCopiedId(null), 2000)
    showToast('Item data disalin')
  }

  const handleCopyItem = (itemData: string, id: string) => {
    // Build a customer-ready paste with product name + item data + notes (as "Ketentuan Produk")
    const item = items.find(i => i.id === id)
    const product = selectedProductData

    const lines: string[] = []
    if (product?.nama) {
      lines.push(`📦 *${product.nama}*${product.kode ? ` (${product.kode})` : ''}`)
      lines.push('')
    }
    lines.push(formatItemDataForCopy(itemData))

    // Per-item notes act as the product terms ("Ketentuan Produk")
    ...
    setCopiedId(id)
    setTimeout(() => setCopiedId(null), 2000)
    showToast('Detail item disalin (siap kirim ke customer)')
  }

  const handleShowDetail = async (item: ProductItem) => {
    setShowDetailModal(true)
    setDetailLoading(true)
    setDetailError(null)
    setSelectedDetail({ item, product: selectedProductData ? { id: selectedProductData.id, kode: selectedProductData.kode, nama: selectedProductData.nama } : null, order: null })

    try {
      const response = await fetch(`/api/product-items/detail?itemId=${encodeURIComponent(item.id)}`, { cache: 'no-store' })
      const payload = await response.json()

      if (!response.ok) {
        throw new Error(payload?.error || 'Failed to load item detail')
      }

      setSelectedDetail(payload)
    } catch (error: unknown) {
      console.error('Error loading item detail:', error)
      setDetailError(getErrorMessage(error) || 'Failed to load item detail')
    } finally {
  ...
  const confirmStatusChange = async () => {
    try {
      const statusUpdate = newStatus === 'sold'
        ? {
          status: newStatus,
          order_id: null,
  ...
  const detailItem = selectedDetail?.item
                    ...
                    <tr key={item.id} className={`hover:bg-gray-50 transition ${
                      ...
                      <td className="px-6 py-3">
                        <div className="flex items-center justify-between group">
                          ...
                          <button
                            onClick={() => handleCopyItemDataOnly(item.item_data, item.id)}
                      ...
                      <td className="px-6 py-3 text-center">
                        <div className="flex items-center justify-center gap-3">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-medium rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-100"
                          >
                            Detail
                          </button>
                          {item.status === 'available' && (
                ...
                <div key={item.id} className={`p-4 hover:bg-gray-50 ${
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex gap-2 ml-2">
                      ...
                      <button
                        onClick={() => handleShowDetail(item)}
                        className="px-2.5 py-1 text-xs font-medium rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-100"
                      >
                        Detail
                      </button>
                      {item.status === 'available' && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-gray-900">Detail Product Item</h2>
                <p className="text-sm text-gray-600 mt-1">
                  {selectedDetail?.product?.nama || selectedProductData?.nama || '-'}
                  {(selectedDetail?.product?.kode || detailItem?.product_code) && (
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-mono text-sm text-gray-900 break-all">{detailItem.id}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{formatDateTime(detailItem.created_at)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Updated At</p>
                      <p className="text-sm text-gray-900">{formatDateTime(detailItem.updated_at)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Batch</p>
                      <p className="text-sm text-gray-900">{detailItem.batch || '-'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Sold At</p>
                      <p className="text-sm text-gray-900">{formatDateTime(detailItem.sold_at)}</p>
                    </div>
                  </div>

                  <div>
                    <p className="text-xs text-gray-500 mb-2">Item Data</p>
                    <div className="flex items-start gap-2">
                      <code className="flex-1 bg-gray-100 text-gray-800 p-3 rounded-lg text-sm font-mono whitespace-pre-wrap break-all">
                        {detailItem.item_data}
                      </code>
                      <button
                        onClick={() => handleCopyItem(detailItem.item_data, detailItem.id)}
                        className="px-3 py-2 text-xs font-medium rounded-lg bg-gray-900 hover:bg-gray-800 text-white"
                      >
                        Copy
                      </button>
                    </div>
                  ...
                  {detailItem.status === 'reserved' && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                      <p className="font-semibold text-yellow-900 text-sm mb-2">Reservation</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div>
                          <p className="text-xs text-yellow-700 mb-1">Reserved For Order</p>
                          <p className="font-mono text-yellow-950 break-all">{detailItem.reserved_for_order || '-'}</p>
                        </div>
                        <div>
                          <p className="text-xs text-yellow-700 mb-1">Expires At</p>
                          <p className="text-yellow-950">{formatDateTime(detailItem.reservation_expires_at)}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {detailItem.status === 'sold' && (
                    <div className="border border-blue-200 rounded-lg overflow-hidden">
                      <div className="bg-blue-50 px-4 py-3 border-b border-blue-200">
                        <p className="font-semibold text-blue-950 text-sm">Sold Information</p>
                      </div>
                      {selectedDetail?.order ? (
                        <div className="p-4 space-y-4">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="font-mono text-sm font-semibold text-indigo-700">{selectedDetail.order.order_id}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500 mb-1">Tanggal Sold</p>
                              <p className="text-sm text-gray-900">{formatDateTime(detailItem.sold_at || selectedDetail.order.paid_at || selectedDetail.order.created_at)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500 mb-1">Order Status</p>
                              <p className="text-sm text-gray-900 capitalize">{selectedDetail.order.status || '-'}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500 mb-1">Total Order</p>
                              <p className="text-sm font-semibold text-gray-900">{formatCurrency(selectedDetail.order.total_amount)}</p>
                            </div>
                          </div>

                          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                            <p className="font-semibold text-gray-900 text-sm mb-3">Informasi Pembeli</p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Nama</p>
                                <p className="text-gray-900">{selectedDetail.order.buyer?.name || '-'}</p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Email</p>
                                <p className="text-gray-900 break-all">{selectedDetail.order.buyer?.email || '-'}</p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Phone</p>
                                <p className="text-gray-900">{selectedDetail.order.buyer?.phone || '-'}</p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Telegram</p>
                                <p className="text-gray-900">
                                  {selectedDetail.order.buyer?.username
                                    ? `@${selectedDetail.order.buyer.username}`
                                    : selectedDetail.order.buyer?.telegram_user_id || '-'}
                                </p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <p className="text-gray-900">{selectedDetail.order.payment_method || '-'}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500 mb-1">Transaction ID</p>
                              <p className="font-mono text-gray-900 break-all">{selectedDetail.order.transaction_id || '-'}</p>
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="p-4 bg-gray-50 text-sm text-gray-600">
                          {detailItem.order_id
                            ? 'Order tidak ditemukan di tabel orders, sehingga detail order dan pembeli tidak tersedia.'
                            : 'Item ini berstatus sold tanpa order_id. Kemungkinan status diubah manual oleh admin, jadi detail order dikosongkan.'}
                        </div>
                      )}
                    </div>
                  )}
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    const { error } = await supabase.from('product_items').insert(inserts satisfies ProductItemInsert[])
                    if (error) throw error
