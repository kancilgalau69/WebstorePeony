...
export function formatProductList(products, page, perPage, total) {
  ...
  const items = sortedProducts.map((p, i) => {
    ...
    // Format nomor dibuat literal; katalog dikirim tanpa Markdown parser.
    return `[${num}] ${name} (${stock})`;
  });

  const list = items.length ? items : ['Tidak ada produk.'];

  // Build list tanpa border
  const box = [];
...
export function formatPurchaseHistory(orders, products) {
  ...
  const lines = orders.map((order, i) => {
    ...
    const productName = orderItems.length > 0
      ? orderItems.map(item => `${item.product_name || item.product_code || '-'} x${item.quantity || 1}`).join(', ')
      : (products.find(p => p.kode === order.productCode)?.nama || order.productCode || '-');
    const amount = Number(order.total_amount ?? order.amount ?? 0) || 0;
    const dateValue = order.paid_at || order.created_at || order.timestamp;
    const date = dateValue ? formatDate(typeof dateValue === 'number' ? new Date(dateValue) : dateValue) : '-';
    const status = order.status ? ` | ${order.status}` : '';

    return `${i + 1}. ${productName} | ${formatCurrency(amount)} | ${date}${status}`;
  });

  const visibleLines = [];
  let totalLength = 120;
  for (const line of lines) {
    if (totalLength + line.length > 3600) break;
  ...
  return cardBlock([
    'Riwayat Pembelian',
    ...visibleLines,
    hiddenCount > 0 ? `... dan ${hiddenCount} riwayat lainnya` : null,
    '',
    'Terima kasih sudah berbelanja!',
