...
import { getUserOrders } from '../../database/orders.js';
...
export async function handleMenu(ctx) {
  ...
  const visibleProducts = session.currentCategory
    ? products.filter(p => p.kategori === session.currentCategory)
    : products;

  if (visibleProducts.length === 0) {
    return ctx.reply(`❌ Belum ada produk untuk kategori: ${session.currentCategory || '-'}`);
  }
...
export async function handleHistory(ctx) {
  ...
  try {
    ...
    console.error('[HISTORY] Failed to load orders from database:', error?.message || error);
    orders = getUserPurchaseHistory(userId, 100);
  }
  const allProducts = getAllProducts();
...
export async function handleTextMessage(ctx) {
  ...
  if (text.length >= BOT_CONFIG.SEARCH_MIN_LENGTH && !text.startsWith('/')) {
    ...
    await ctx.reply(formatSearchResults(results, text), searchResultsKeyboard(results.length));
  }
}
