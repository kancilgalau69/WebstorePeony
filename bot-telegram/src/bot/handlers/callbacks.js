...
async function handleMenuCallback(ctx, params) {
  ...
  if (menuType === 'main') {
    updateUserSession(ctx.from.id, { currentCategory: null, currentPage: 1 });
...
async function handleViewCallback(ctx, params) {
  ...
  if (source === 'search') {
    ...
    const visibleProducts = session.currentCategory
      ? allProducts.filter(p => p.kategori === session.currentCategory)
      : allProducts;
...
async function handleCategoryCallback(ctx, params) {
  ...
  if (categoryParam === 'all') {
    ...
    const categoryIndex = Number.parseInt(categoryParam, 10);
