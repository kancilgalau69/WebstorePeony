...
// Debounce: prevent double-tap on buy button (per-user, 3 second window)
const PURCHASE_DEBOUNCE = new Map(); // userId -> timestamp
const PURCHASE_DEBOUNCE_MS = 3000;
...
export async function handlePurchase(ctx, productCode, quantity = 1) {
  ...
  // Debounce: ignore if same user triggered purchase within 3 seconds
  const now = Date.now();
  const lastPurchaseTime = PURCHASE_DEBOUNCE.get(userId);
  if (lastPurchaseTime && (now - lastPurchaseTime) < PURCHASE_DEBOUNCE_MS) {
    // Silently ignore duplicate tap
    if (ctx.callbackQuery) {
      try { await ctx.answerCbQuery('⏳ Pesanan sedang diproses...'); } catch {}
    }
    return;
  }
  PURCHASE_DEBOUNCE.set(userId, now);

  // Cleanup old debounce entries periodically (prevent memory leak)
  if (PURCHASE_DEBOUNCE.size > 1000) {
    for (const [uid, ts] of PURCHASE_DEBOUNCE.entries()) {
      if (now - ts > PURCHASE_DEBOUNCE_MS * 2) PURCHASE_DEBOUNCE.delete(uid);
    }
  }
